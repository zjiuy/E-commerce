...
from django.shortcuts import  render
# 初始化 Neo4j 连接（请根据你的配置修改）
graph = GraphDatabase.driver("bolt://localhost:7687", auth=("neo4j", "z@130425"))  # 替换为你的用户名和密码
from django.http import JsonResponse


def get_allRela(request):
    # 初始化关系列表
    relations = []
#     MATCH(m) - [r]->(n:`榜单类型` {name: '茗茶'})
#
#
# RETURN
# m, TYPE(r) as rt, n
...
def commentsChar(request):
    ...
    year, mon, day = getHomeData.getNowTime()
    # 读取 Excel 数据
    excel_path = r"D:\data.xlsx"
    df = pd.read_excel(excel_path)

    # 获取所有榜单类型
    category_list = df['榜单类型'].unique().tolist()

    # 根据请求参数筛选数据
    category = request.GET.get('category', None)

    if category:
        df_filtered = df[df['榜单类型'] == category]
    else:
        df_filtered = df

    # 按品牌聚合销售额和销售数量
    brand_sales = df_filtered.groupby('INSRE').agg({
        '周销售额': 'sum',
        '周销售数量': 'sum',
        '市场价': 'mean',
        '券后价': 'mean'
    ...
    # 准备 Echarts 数据
    brand_list = brand_sales['INSRE'].tolist()
    sales_list = brand_sales['周销售额'].tolist()
    quantity_list = brand_sales['周销售数量'].tolist()
    market_price_list = brand_sales['市场价'].tolist()
    coupon_price_list = brand_sales['券后价'].tolist()
    ...
    # 按销售额降序排序
    top_brands.sort(key=lambda x: x['sales'], reverse=True)
    # 获取前十名品牌
    ...
    data = {
        'xData': brand_list,
        'ySalesData': sales_list,
        'yQuantityData': quantity_list,
        'marketPriceData': market_price_list,
        'couponPriceData': coupon_price_list,
